...
- 联合使用三种扭结不变量将 1783 种扭结不变量划分为了 1747 个等价类。
  - 其中包含一元等价类 1711 个。
  - 二元等价类 36 个。
...
## 使用 volume 区分非素扭结（**不建议使用**）

- 尽管非素扭结都是 non-hyperbolic 的，正如前文所述，我们仍然可以使用 snappy 中的 volume 函数得到一个计算结果。
- 但需要明确的是，**这种行为只是一种工程上的权宜的做法**，缺乏相关的数学基础，因此**不建议使用**。
- 由于 volume 完全没有手性区分的能力，因此往往需要结合 khovanov 同调以及 HOMFLY-PT 一同使用。
- 在早期的程序中确实使用了这种方法用于区分非素扭结，因而我们使用本节内容概括 volume 对非素扭结的区分能力在 khovanov 以及 HOMFLY-PT 工作的基础上的增强程度。

### 联合使用 khovanov 同调以及 HOMFLY-PT 多项式区分非素扭结

- 在小于等于 11 crossing 的扭结中，我们能够找到非素扭结总计 200  种。
- 我们联合使用 khovanov 同调、HOMFLY-PT 多项式/以及 volume 计算结果（保留小数点后前三位结果），试图将这 200 种不同非素扭结划分为若干等价类。
- 实验表明，这 200 种非素扭结将会被划分为 200 个互不相交的等价类。
